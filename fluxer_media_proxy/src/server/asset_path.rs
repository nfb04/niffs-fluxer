...
pub(in crate::server) fn parse_guild_soundboard_sound_path(path: &str) -> Option<String> {
    let mut parts = canonical_public_path(path)?.split('/');
    ...
    let (sound_id, ext) = filename.split_once('.')?;
    ...
    Some(format!("soundboard/{guild_id}/{filename}"))
}
...
mod tests {
    ...
    #[test]
    fn guild_soundboard_sound_path_parses_valid_keys() {
        for ext in ["mp3", "wav"] {
            assert_eq!(
                parse_guild_soundboard_sound_path(&format!(
                    "/soundboard/1130650140672000000/987654321.{ext}"
                )),
                Some(format!("soundboard/1130650140672000000/987654321.{ext}"))
            );
        }
    }

    #[test]
    fn guild_soundboard_sound_path_rejects_invalid_keys() {
        assert_eq!(
            parse_guild_soundboard_sound_path("/soundboard/42/99.ogg"),
            None
        );
        assert_eq!(
            parse_guild_soundboard_sound_path("/soundboard/abc/99.mp3"),
            None
        );
        assert_eq!(
            parse_guild_soundboard_sound_path("/soundboard/42/abc.mp3"),
            None
        );
        assert_eq!(
            parse_guild_soundboard_sound_path("/soundboard/42/99.mp3/x"),
            None
        );
        assert_eq!(
            parse_guild_soundboard_sound_path("/soundboard/42/../secret.mp3"),
            None
        );
        assert_eq!(parse_guild_soundboard_sound_path("/soundboard/42"), None);
        assert_eq!(
            parse_guild_soundboard_sound_path("/soundboard//99.mp3"),
            None
        );
        assert_eq!(
            parse_guild_soundboard_sound_path("/avatars/42/99.mp3"),
            None
        );
    }
